...
function getMiddleware(connection) {
            ...
            if (ctx.query.group) {
                ...
                if (format !== 'csv') {
                    ...
                    if (ctx.query['page[size]']){
                        try {
                            size = parseInt(ctx.query['page[size]']); 
                        } catch(err){}
                    }
                    if (ctx.query['page[index]']){
                        try {
                            page = parseInt(ctx.query['page[index]']); 
                            if (page < 1){
                                page = 1;
                            }
                        } catch(err){}
                    ...
                    ctx.body = await StadisticModel.find(filter).sort(ctx.query.sort || '-date').skip(size * (page -1)).limit(size).exec();
                } else {
                    ctx.body = await StadisticModel.find(filter).sort(ctx.query.sort || '-date');
                }
            }
            if (format === 'csv') {
                const parser = new Json2csvParser();
                ctx.body = parser.parse(ctx.body.map(o => o.toObject()));
    ...
    ApiRouter.get('/requestByDay', isAdmin, StadisticRouter.requestByDay);
    ApiRouter.get('/countRequegstTodayByCountry', isAdmin, StadisticRouter.countRequestTodayByCountry);
    ApiRouter.get('/countRequestToday', isAdmin, StadisticRouter.countRequestToday);
    ApiRouter.get('/countRequestLastWeek', isAdmin, StadisticRouter.countRequestLastWeek);
    ApiRouter.get('/timeByRequest', isAdmin, StadisticRouter.timeByRequest);
    ApiRouter.get('/avgByRequest', isAdmin, StadisticRouter.avgByRequest);
    ApiRouter.get('/', isAdmin, StadisticRouter.get);
